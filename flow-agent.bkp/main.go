...
type SASLServerFactory func(net.Conn) sasl.Server
...
func server() error {
	...
	endpoint.AddHandleFunc("auth", handleAuth)
...
func NewEndpoint() *Endpoint {
	...
	return &Endpoint{
		...
		// auths:   nil,
		auths: map[string]SASLServerFactory{
			sasl.Plain: func(conn net.Conn) sasl.Server {
...
func handleAuth(e *Endpoint, conn net.Conn, rw *bufio.ReadWriter, scanner *bufio.Scanner) {
	...
	saslServer := e.auths[sasl.Plain](conn)
	log.Println(&saslServer)
	conn.Close()
	return
}
